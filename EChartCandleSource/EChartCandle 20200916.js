...
/*       myChart.dispatchAction({
      ...
      myChart.on("contextmenu" , function (params) {         
        ...
        var selector =[] , isAttributeSelector = true ;  
        var graphicModels = []; 
        if (params.dataType === "node")   {
          selector.push( params.data.selector ) ; 
          isAttributeSelector = true  ; 
        }
        else if (params.dataType === "edge")   {
          selector = params.data.metricSelector ; 
        ...
        for (var i=0 ; i<selector.length ; i++) {
          ...
          graphicModel.setSelector ( selector[i], isAttributeSelector ) ; 
          graphicModels.push(graphicModel) ;
        }

          for (i=0 ; i<graphicModels.length; i++) {
            me.handleSelection({ctrlKey:true , metaKey:true} , graphicModels[i]) ; 
           //me.handleSelection(evt , graphicModels[i]) ; 
          }
          // me.handleSelection ( graphicModel, true ) ;
          me.handleContextMenuSelection(evt , graphicModels[0] ) ; 
      }) ;  
      ...
      myChart.on("brushEnd", function (params) {      
          selectorHandling (params.areas[0].coordRange) ; 
      }) ;
